...
make_valid_fcs<-function(exprs,fcs=NULL,desc1=NULL){
  require(flowCore)
  if(!is.null(fcs)) fcs<-flowFrame(exprs, parameters = parameters(fcs)) else fcs<-flowFrame(exprs)
  ...
  if(!is.null(fcs)) fcs<-flowFrame(exprs, parameters = params) else fcs<-flowFrame(exprs,parameters = params)
  keyword(fcs) <- keyval
